...
inline void aegis_state_update(aegis_state *cs, uint32_t *message_block)
{
	...
	// Store s0 as tmp var.
	B_TMP = vld1q_u8((uint8_t*)cs->s0);
	B_MSG = vld1q_u8((uint8_t*)message_block);
	B_KEY = veorq_u8(B_TMP, B_MSG);
	B_S0 = vld1q_u8((uint8_t*)cs->s4);
#else
	...
	//vst1q_u8((uint8_t*)cs->s2, B);
	//B = vaeseq_u8(B, Key);
	B_S0 = vaesmcq_u8(vaeseq_u8(B_S0, B_KEY));
	B_S0 = veorq_u8(B_S0, B_KEY);
	vst1q_u8((uint8_t*)cs->s0, B_S0);
#else	
